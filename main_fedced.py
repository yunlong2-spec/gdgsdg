...
def model_aggregation(selected_clients, global_model, trust_scores):
    ...
    # 计算平均信任分数
    s_avg = sum(trust_scores) / len(trust_scores)
    ...
    for client, trust_score in zip(selected_clients, trust_scores):
        ...
        for name, update in local_update.items():
            trust_adjusted_update[name] = update * (1 + client['trust_score'] - s_avg)

        # 模型更新一致性检查
        D_i_t = 0.0
        for name, global_param in new_global_model.named_parameters():
            if name in trust_adjusted_update:
                D_i_t += torch.norm(trust_adjusted_update[name] - (global_param.data - global_model.state_dict()[name])) / torch.norm(global_param.data - global_model.state_dict()[name])
        D_i_t /= len(trust_adjusted_update)
        ...
        # 更新信任分数
        client['trust_score'] += trust_adjusted_update - D_i_t

        # 模型聚合
        for name, global_param in new_global_model.named_parameters():
            if name in trust_adjusted_update:
                global_param.data += (trust_score / total_weight) * trust_adjusted_update[name]
...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):
        ...
        for device in devices:
            ...
            w, loss = local.train(net=device['model'].to(args.device))  # 确保模型在GPU上
            device['model'].load_state_dict(w)  # 确保每个设备都保持其训练后的模型
        ...
        # 更新全局模型
        net_glob.load_state_dict(new_global_model.state_dict())  # 使用更新后的全局模型权重
