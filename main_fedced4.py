...
from utils.options1 import args_parser
from models.Update2 import LocalUpdate
from models.Nets2 import MLP, CNNMnist, CNNCifar
from models.test3 import test_img
...
def calculate_device_parameters(data):
    ...
    for id, device_data in data.items():
        ...
        T_cmp_N1 = max([(c_n * len(data[j])) / f_max for j in data if len(data[j]) > 0])  # 避免除以零
        T_cmp_N2 = max([(c_n * len(data[j])) / f_min for j in data if len(data[j]) > 0])  # 避免除以零
        T_cmp_N3 = (sum([alpha_n * (c_n * len(data[j])) ** 3 for j in data if len(data[j]) > 0]) / kappa) ** (1 / 3)
...
# 聚合客户端模型
def model_aggregation(selected_clients, global_model, trust_scores=None):
    """
    模型聚合函数，支持 FedAvg 和基于信任分数的加权聚合。
    如果 trust_scores 为 None，则使用 FedAvg；否则使用信任分数进行加权聚合。
    """
    if trust_scores is None:
        # 使用 FedAvg 聚合
        w = [client['model'].state_dict() for client in selected_clients]
        new_global_model = FedAvg(w)
    ...
    for client, trust_score in zip(selected_clients, trust_scores):
        ...
        for name, update in local_update.items():
            trust_adjusted_update[name] = update * (1 + trust_score - S_avg)
        ...
        D_max = D_max + np.mean(d_i_t_values_cpu) + 2 * np.std(d_i_t_values_cpu)  # 可根据实际情况评估
        ...
        for name in new_global_model.state_dict().keys():
            if name in trust_adjusted_update:
                new_global_model.state_dict()[name].data += trust_adjusted_update[name]
...
if __name__ == '__main__':
    ...
    # 创建验证数据加载器
    validation_loader = DataLoader(dataset_test, batch_size=50, shuffle=False)
